...
当前版本仍处于早期开发阶段，功能以多标签终端工作台、自动日志、快速绘图和设置为主。
...
11. 可新增标签页同时调试多个串口，新会话会复制当前配置。
12. 可点击左侧卡片标题折叠不常用配置，减少侧栏占用。
13. 可开启自动断帧、时间戳、自动滚动、循环发送和自动日志等选项。
14. 使用 TXT、CSV、BIN 导出会话记录。
