...
当前版本仍处于早期开发阶段，功能以多标签终端工作台、自动日志、快速绘图、数据表格、脚本插件和设置为主。
...
- 左侧卡片：连接、接收、发送、Modbus、常用包、宏命令、自动应答和文件发送卡片可折叠，并记住折叠状态。
...
8. 可在脚本插件中载入或编写 JavaScript，通过 `serial.sendText()`、`serial.sendHex()`、`serial.records()` 和 `serial.log()` 自动化调试流程。
9. 可在自动应答中保存文本、HEX 或正则匹配规则，命中后立即或延时发送指定应答。
10. 可选择文件，配置块大小和间隔后分块发送。
11. 可打开快速绘图窗口，按全部数字、分隔值、键值对或 JSON 对象协议从接收文本取数并导出 CSV。
12. 可打开数据表格窗口，按帧排序、过滤、复制记录，并定位回终端对应记录。
13. 可新增标签页同时调试多个串口，新会话会复制当前配置。
14. 可点击左侧卡片标题折叠不常用配置，减少侧栏占用。
15. 可开启自动断帧、时间戳、自动滚动、循环发送和自动日志等选项。
16. 使用 TXT、CSV、BIN 导出会话记录。
