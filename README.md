...
- 终端检索：支持按关键字搜索，并按全部、接收、发送过滤当前显示。
...
- 左侧卡片：连接、接收、发送、Modbus、常用包和文件发送卡片可折叠，并记住折叠状态。
...
- 导出记录：支持 TXT、CSV、BIN。
- 接收保存：可将接收原始数据保存到文件。
- 会话恢复：恢复上次串口参数、发送输入、常用包、文件发送参数，可选启动后自动连接。
...
│           ├── *_layout.cpp              # 页面整体布局
│           ├── *_side_sections.cpp       # 左侧连接、收发、常用包、文件发送配置
...
6. 可在常用包中填写名称、内容、模式和换行后保存，之后选中条目即可填入发送区或直接发送。
7. 可选择文件，配置块大小和间隔后分块发送。
8. 可点击左侧卡片标题折叠不常用配置，减少侧栏占用。
9. 可开启自动断帧、时间戳、自动滚动、循环发送等选项。
10. 使用 TXT、CSV、BIN 导出会话记录。
